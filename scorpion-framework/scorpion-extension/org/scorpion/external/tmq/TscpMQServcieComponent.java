...
@Component(name="TscpMQServcieComponent",params={"brokerConfig=tscp-mq-conf.xml"}, sequence = 46)
public class TscpMQServcieComponent extends AbsTscpComponent{
	...
	private void parseDocument(String configFile) throws DocumentException, IOException{
		...
		if(!conf.exists())
			throw new FileNotFoundException("TSCP-4098:File ["+conf.getPath()+"] no found !");
		
		Document document = DocumentHelper.parseText(TscpFileStreamHandler.getFileContent(conf.getPath()));
