 ...
 */
public class TscpFileRandomAccessReader extends AbsDataRandomAccess{
	 ...
	 */
	public TscpFileRandomAccessReader(int cachesize,String filePath) throws TscpBaseException{
		
		if(filePath == null)
			throw new TscpBaseException("TSCP-4907：The cutting file don't allow to be null !");
	 ...
	 * @throws TscpBaseException
	 */
	public TscpFileRandomAccessReader(String filePath) throws TscpBaseException{
	 ...
	 * @throws TscpBaseException
	 */
	public TscpFileRandomAccessReader(File file) throws TscpBaseException{
	...
	public Map<String,Object> randomAccessread(int size) throws IOException, InterruptedException {
		...
		if(cachesize == 0){
			byte[] b = TscpDataHandler.cutFileToPieceByOffset(file.getPath(), lastOffset, size);
			if(b == null)
				return null;
	...
	public synchronized Map<String,Object> randomAccessread(int offset, int size) throws IOException, InterruptedException {
		...
		if(cachesize == 0){
			...
			byte[] _b = TscpDataHandler.cutFileToPieceByOffset(file.getPath(), offset, size);
