...
import com.scorpion.huakerongtong.common.mdb.ConvertCacheData;
import com.scorpion.huakerongtong.common.util.ScorpionKVMemoryCacheUtil;
import com.scorpion.huakerongtong.persistence.cache.CacheConf.CacheRef;
import com.scorpion.huakerongtong.persistence.cache.CacheConf.ViewItem;
import com.scorpion.huakerongtong.persistence.util.ScorpionPersistenceUtils;
 ...
 */
public class ScorpionDBCacheComponent extends AbsScorpionComponent{
	
	
	private Map<String,CacheConf> sqlmap = new HashMap<String,CacheConf>();
	
	
	private String DEFUALT_CACHE_FILE = "scorpion-db-cache.xml";
	...
	private boolean loadCacheConfig(String confFile) throws ScorpionBaseException{
		
		if(!new File(this.getClass().getResource("/"+confFile).getPath()).exists()){
			PlatformLogger.warn("scorpion-4976: System cache file [ scorpion-db-cache.xml ] no found .....");
			return false;
		}
		try {
			Document docuemnt = DocumentHelper.parseText(ScorpionFileStreamHandler.getFileContent(this.getClass().getResource("/"+confFile).getPath()));
			...
			//	String dsname = ele.attributeValue(""); String freshTime = ele.attributeValue("");
				...
				if(isCache == null|| "".equals(isCache)||!Boolean.parseBoolean(isCache)) 
					ScorpionKVMemoryCacheUtil.getRdKvCachePersistence().clearAll();
					...
					for(Element che:ches){
						...
						for(Element elre:elres){
							...
							if(cacheRef.getSourceCode() == null||"".equals(cacheRef.getSourceCode())||cacheRef.getTargetCode() == null||
								...
								throw new ScorpionBaseException("请检查配置文件["+DEFUALT_CACHE_FILE+"]属性 sourceCode  targetCode sql caption 配置.....");
			...
			throw new ScorpionBaseException("scorpion-2144: Loading database cache file fail ...",e);
		}
	 ...
	 */
	private void loadDbDataToCache() throws ScorpionBaseException{
	 ...
	 * @throws ScorpionBaseException
	 */
	private void cacheTruckData(String keyCache,String sql,CacheConf cacheConf) throws ScorpionBaseException{
		
		IScorpionKVCache kv = null;
		try{
			kv = ScorpionKVMemoryCacheUtil.getKVMemoryPersistence();
			List<Map<String,Object>> lismap = ScorpionPersistenceUtils.getPersistenceService().queryLisMapBySQL(sql, null);
			...
			if(keyCache.split("#").length>1){
				if(keyCache.split("\\.").length>1){
					...
					throw new ScorpionBaseException("不支持键值混合配对");
				}
			}else{
				if(keyCache.split("\\.").length>1){
	 ...
	 * @throws ScorpionBaseException
	 */
	private void cacheRefData(IScorpionKVCache kv,List<Map<String,Object>> lismap,String[] key,List<ViewItem> viewItems,boolean isRow) throws ScorpionBaseException{
		
		for(ViewItem viewItem:viewItems){
			for(CacheRef cacheRef:viewItem.getCacheRefs())
				if(isRow)
					ConvertCacheData.convertCodeToCaption(kv,key,lismap,ScorpionPersistenceUtils.getPersistenceService().queryLisMapBySQL(cacheRef.getSql(), null),
							cacheRef.getSourceCode(),cacheRef.getTargetCode(),cacheRef.getCaption());
				else
					ConvertCacheData.convertCodeToCaption(null,null,lismap,ScorpionPersistenceUtils.getPersistenceService().queryLisMapBySQL(cacheRef.getSql(), null),
							cacheRef.getSourceCode(),cacheRef.getTargetCode(),cacheRef.getCaption());
