 ...
 */
public class TscpSystemScanInfo {
...
/*	public class ComponentInfo{
		...
		public int getComponentStatue() throws TscpBaseException {
	
			if(component != null)
				throw new TscpBaseException("TSCP-6894:组件["+this.getName()+"]未启动！");
		
			return component.getComponentstate();
		}

		public void setComponentStatue(int componentStatue) throws TscpBaseException {
		...
		public AbsTscpComponent getComponents() {
			return component;
		}
	 ...
	 */
	public ServiceInfo getService(String serviceName)throws TscpBaseException{
		  if(!servicePool.containsKey(serviceName))
			  throw new TscpBaseException("TSC-9906:调用服务["+serviceName+"]不存在!");
	 ...
	 * @throws TscpBaseException
	 */
	public ActionInfo getActionByName(String actionName)throws TscpBaseException{
		  if(!actionPool.containsKey(actionName))
			  throw new TscpBaseException("TSC-9907:调用Action不存在!");
	 ...
	 * @throws TscpBaseException 
	 */
	public Method getActionMethod(String actionName,String methodName) throws TscpBaseException{
		
		if(actionPool.get(actionName) == null||actionPool.get(actionName).getMethods().get(methodName) == null)
			throw new TscpBaseException("TSCP-6009:ACTION["+methodName+"]不存在，检查ACTION名称是否正确!");
